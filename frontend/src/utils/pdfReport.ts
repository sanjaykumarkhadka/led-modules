/**
 * PDF Report Generator for Led-modules LED Configurator
...
/**
 * Draw the Led-modules logo header
 */
function drawHeader(doc: jsPDF, pageWidth: number): number {
  const marginLeft = 15;

  // Draw "Led-modules" text logo
  doc.setFontSize(28);
  doc.setTextColor(COLORS.darkBlue);
  doc.setFont('helvetica', 'bold');
  doc.text('Led-modules', marginLeft, 18);
  ...
  doc.text('support@led-modules.local', centerX, 25, { align: 'center' });
...
export async function generatePDFReport(data: ReportData): Promise<void> {
  ...
  doc.text('Generated by Led-modules LED Configurator', pageWidth / 2, pageHeight - 8, {
    align: 'center',
  });
