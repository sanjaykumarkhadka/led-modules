...
export function DesignerPage() {
  ...
  const handleSaveProject = async () => {
    if (!projectName.trim()) {
      const name = window.prompt('Enter a project name:', projectName || '');
      if (!name) return;
      setProjectName(name);
      await saveCurrentProject(name, projectDescription);
    } else {
        ...
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex flex-col gap-1">
            <input
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-700 text-xs text-slate-100 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
            <input
              type="text"
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-[11px] text-slate-300 focus:outline-none focus:ring-1 focus:ring-slate-600"
            />
          </div>
