...
class LEDConfigurator {
  ...
  handleArtworkUpload(file) {
    ...
    this.state.uploadedFileType = fileName.endsWith('.pdf')
      ? 'pdf'
  ...
  calculateCustomArtworkResults(totalLEDs, numElements = 1, ledsPerElement = 0) {
    ...
    const selectedModule = CONFIG_DATA.modules[document.getElementById('module').value];
    const selectedPowerSupply =
      CONFIG_DATA.powerSupplies[document.getElementById('powerSupply').value];
  ...
  async renderText() {
    ...
    document.querySelectorAll('.character-group').forEach((charGroup) => {
      ...
      if (outlinePath && char) {
        ...
        console.log(
          `Placing ${charLedCount} LEDs for character '${char}', bbox:`,
          outlinePath.getBBox()
        );
  ...
  showCharacterLEDControls(charGroup) {
    const char = charGroup.getAttribute('data-char');
    const currentLedCount =
      this.state.ledCountPerChar[char] || parseInt(document.getElementById('ledCount').value);
  ...
  selectLED(led) {
    ...
    if (ledControlPanel) {
      ...
      document.getElementById('individualLedColor').value =
        led.getAttribute('data-color') || '#666666';
  ...
  calculateResults() {
    ...
    const recommendedSpacingValue = document.getElementById('unitToggle').checked
      ? (avgSpacing * 2.54).toFixed(1)
      : avgSpacing.toFixed(1);
    document.getElementById('recommendedSpacing').textContent =
      `${recommendedSpacingValue} ${unit}`;

    const circuitLengthValue = totalPerimeter * 12 * 1.2;
    const circuitLengthDisplay = document.getElementById('unitToggle').checked
      ? (circuitLengthValue * 2.54).toFixed(1)
      : circuitLengthValue.toFixed(1);
      ...
      img.onload = () => {
        ...
        const specs = [
          ...
          [
            ...
            document.getElementById('powerSuppliesCount')?.textContent || '0',
          ],
        ];
        ...
        for (const char of this.state.currentText) {
          if (char !== ' ') {
            const ledCount =
              this.state.ledCountPerChar[char] ||
              parseInt(document.getElementById('ledCount').value);
